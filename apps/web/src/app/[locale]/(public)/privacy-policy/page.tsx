...
export async function generateMetadata({
  ...
  const route = locale === 'es' ? '/privacy-policy' : `/${locale}/privacy-policy`
  const [seo, t] = await Promise.all([fetchSEO(route), getTranslations('privacyPolicy')])
...
export default async function PrivacyPolicyPage() {
  ...
  const sections = [
    ...
    { id: 'section-5', title: t('s6Title'), content: t('s6Content') },
  ]
