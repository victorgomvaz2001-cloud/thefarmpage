...
export async function generateMetadata({
  ...
  const route = locale === 'es' ? '/cookies-policy' : `/${locale}/cookies-policy`
  const [seo, t] = await Promise.all([fetchSEO(route), getTranslations('cookiesPolicy')])
  return buildMetadata(seo, { title: t('title') })
}

export default async function CookiesPolicyPage() {
  const [t, tl] = await Promise.all([
    getTranslations('cookiesPolicy'),
